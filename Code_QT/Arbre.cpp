...
fclose(file);
  ...
  void Arbre::loadTXT (const std::string & filename) {
     ...
     for (int k=37; k<43; k++) {
          for (int i=0; i<84; i++) {
              fscanf(file, "%f",&N[i]->rotation[k][0]);
              fscanf(file, "%f",&N[i]->rotation[k][1]);
              fscanf(file, "%f",&N[i]->rotation[k][2]);
          }
   ...
   /*  for (int k=1; k<7; k++) {
         for (int i =0; i<84;i++){
         N[i]->p[36+k] = (float)(7-k) * N[i]->p[36] / 7 + (float)(k) * N[i]->p[43] / 7;
         }
     }
      ...
      for (int i =0; i<84;i++)
      {
      N[i]->p[15] = (float)1* N[i]->p[10] /2 + (float)1 * N[i]->p[20] /2;
      }
