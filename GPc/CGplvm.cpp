      ...
      pnoise(nois), latentDim(latDim), KupToDate(false)
{
    ...
    initStoreage();
    initVals();
    cout << "In line 90 of CGPLVM, end of initialization" << endl;
}
...
double CGplvm::logLikelihood() const
{
    ...
    {
        cout<<"USING DYNAMIC MODEL IN LINE 525 OF CGplvm.cpp"<<endl;
...
void CGplvm::optimise(const int iters)
{
    if(getVerbosity()>2)
    {
        ...
        display(cout);
        cout<< "in line 744 of CGplvm.cpp, verbosity is: "<< getVerbosity()<<endl;
    }
    if(getVerbosity()>2 && getOptNumParams()<40)
        checkGradients();
    setMaxIters(iters);
    cout<<"in line 748 of CGplvm.cpp, before runDefaultOptimiser"<<endl;
    runDefaultOptimiser();

    if(getVerbosity()>1)
        cout << "... done. in line 753  verbosity = "<<getVerbosity() << endl;
    if(getVerbosity()>0){
        display(cout);
    }
}
